...
function local_obu_application_get_course_dates($timestamp = null) {
	...
	while (count($dates) < 5) {
		...
		if ($months[$tempmonthindex] == 'SEP') {
			$dates[$months[$tempmonthindex] . $year] = "Autumn " . $displayyear . " (Sem 1)";
		} elseif ($months[$tempmonthindex] == 'JAN') {
			$dates[$months[$tempmonthindex] . $year] = "Spring " . $displayyear . " (Sem 2)";
		} elseif ($months[$tempmonthindex] == 'JUN') {
			$dates[$months[$tempmonthindex] . $year] = "Summer " . $displayyear . " (Sem 3)";
		}
...
function local_obu_application_get_application_status_row_html($label, $type, $state) {
	...
	if ($type == 'past') {
		$text .= "<div class='row text-primary mb-1'>";
	}
	else if ($type == 'future') {
		$text .= "<div class='row text-muted mb-1'>";
	}
	else if($type == "xfer_first") {
		$text .= "<div  class='row text-muted mt-2 mb-1'>";
	}
	else if($type == "xfer") {
...
function local_obu_application_update_workflow(&$application, $approved = true, $data = null) {
	...
	if (isset($data->qualification_verified) && is_numeric($data->qualification_verified)) {
		...
		local_obu_application_update_applicant($application->userid, $application->qualification_verified);
	}
...
function local_obu_application_update_applicant($userid, $verified){
	global $DB;
